...
/**
  * Creating a json representation of calculation method functions data
  */
class CalcMethodFunctionsJsonFormat extends BasicJsonFormat {

  implicit val calcMethodFunctionWrites = Json.writes[CalcMethodFunction]

  implicit val calcMethodFunctionResponseWrites: Writes[CalcMethodFunctionResponse] = (
    (JsPath \ ApiConstants.CONTEXT_NAME).write[URL] and
      ...
      (JsPath \ ApiConstants.DATA_NAME).write[Seq[CalcMethodFunction]]
    )(unlift(CalcMethodFunctionResponse.unapply))
  ...
  /**
    ...
    */
  def format[A](start: DateTime, calcMethodFunctions: List[CalcMethodFunction])(implicit request: Request[A]): String = {
    val size = calcMethodFunctions.size
    ...
    val response = new CalcMethodFunctionResponse( new URL(ApiConstants.METAPI_CONTEXT),
